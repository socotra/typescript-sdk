...
test('should typecheck', () => {
    ...
    const NotificationSchemaV3 = z.object({
        method: z.string(),
        params: z.optional(z.object({ _meta: z.optional(z.object({})) }).passthrough())
    });

    const ResultSchemaV3 = z.object({}).passthrough();

    // Now extend with v3 schemas using z.literal and z.object from zod/v3
    const GetWeatherRequestSchema = RequestSchemaV3.extend({
    ...
    const GetForecastRequestSchema = RequestSchemaV3.extend({
    ...
    const WeatherForecastNotificationSchema = NotificationSchemaV3.extend({
    ...
    const WeatherResultSchema = ResultSchemaV3.extend({
