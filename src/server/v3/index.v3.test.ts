...
/*
  ...
  This test demonstrates how a Zod v3 user would create and use schemas with the SDK.
  */
test('should typecheck', () => {
    // Define base v3 schemas that mirror the SDK's structure
    const RequestSchemaV3Base = z.object({
        method: z.string(),
        params: z.optional(z.object({ _meta: z.optional(z.object({})) }).passthrough())
    });

    const NotificationSchemaV3Base = z.object({
        method: z.string(),
        params: z.optional(z.object({ _meta: z.optional(z.object({})) }).passthrough())
    });

    const ResultSchemaV3Base = z.object({}).passthrough();

    // Now extend with v3 schemas using z.literal and z.object from zod/v3
    const GetWeatherRequestSchema = RequestSchemaV3Base.extend({
    ...
    const GetForecastRequestSchema = RequestSchemaV3Base.extend({
    ...
    const WeatherForecastNotificationSchema = NotificationSchemaV3Base.extend({
    ...
    const WeatherResultSchema = ResultSchemaV3Base.extend({
